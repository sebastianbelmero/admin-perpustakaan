<div>
    ...
    id = {{  $idnya }}
    @if ($idnya)
	<button wire:click="showFormKategori({{$idnya}})" class="px-2 py-1 bg-blue-500 hover:bg-blue-700 rounded text-white"><i class="bi bi-pencil"></i> Edit</button>
	...
	@if ($idnya !== $updateId)
			@livewire('pages.tambah-kategori')
	@endif
	@if ($idnya === $updateId)
	<div>
		<form wire:submit.prevent="ubahKategori({{ $idnya }})">
			<div class="flex flex-wrap items-center">
				<div class="w-full lg:w-3/4 p-1">
					<input wire:model.defer ="nama" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" type="text">
				</div>
